    ...
    <section class="min-w-0 w-full max-w-full rounded-lg border border-neutral-300 bg-white p-4 shadow-sm">
      ...
      <div class="mt-4 grid min-w-0 gap-3">
        <button
          ...
          type="button"
        >
          {busy === 'allocation' ? 'Checking allocation' : walletAddress ? 'Check $BORO allocation again' : 'Connect and check $BORO allocation'}
        </button>
        ...
        {missingConfig.length > 0 && (
          <div class="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm leading-6 text-amber-950">
            Base mainnet $BORO claims are waiting on backend configuration.
          </div>
        )}
