...
import { CLAIM_COPY } from './claim-copy'
...
export function App() {
  ...
  const claimsEnabled = config?.enabled === true
  const canClaim = Boolean(allocation?.eligible && recipient && claimsEnabled)

  const status = useMemo(() => {
    if (!claimsEnabled) return 'offline'
    ...
    return 'connect'
  }, [allocation, claim, claimsEnabled, walletAddress])
  ...
  return (
    <section class="min-w-0 w-[calc(100vw-2rem)] rounded-lg border border-neutral-300 bg-white p-4 shadow-sm md:w-auto">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 class="text-xl font-semibold">{CLAIM_COPY.title}</h1>
          <p class="mt-1 text-xs font-medium uppercase text-neutral-500">{CLAIM_COPY.network}</p>
        </div>
      ...
      <div class="mt-4 grid min-w-0 gap-3">
        <button
          ...
          disabled={!claimsEnabled || busy === 'allocation'}
          onClick={checkAllocation}
          type="button"
        >
          {busy === 'allocation' ? 'Checking allocation' : walletAddress ? 'Check allocation again' : 'Connect Solana wallet'}
        </button>

        <dl class="grid min-w-0 gap-3 sm:grid-cols-2">
          <Metric label="Network" value={config?.chainName ?? CLAIM_COPY.network} />
          <Metric label="Claim method" value={CLAIM_COPY.method} />
          ...
          <Metric label="Transaction" value={claim?.txHash ? shorten(claim.txHash, 10, 8) : '...'} href={claim?.txHash ? explorerTxUrl(claim.txHash) : undefined} />
        </dl>
        ...
        <button
          ...
          type="button"
        >
          {busy === 'challenge' ? 'Signing claim' : 'Claim $BORO to address'}
        </button>
        ...
        {!claimsEnabled && (
          <div class="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm leading-6 text-amber-950">
            {config?.disabledReason ?? 'Claims are not configured yet.'}
          </div>
        )}
...
function ensureClaimsEnabled(config: ConfigResponse | null) {
  if (config?.enabled) return
  throw new Error(config?.disabledReason ?? 'Claims are not configured yet.')
}
